...
export class UsersManagerTest {
    ...
    async createRandomUser(): Promise<{ userId: string }> {
        const configService = this.app.get(ConfigService);
        const apiSettings = configService.get<ApiSettings>('apiSettings');
        const userName = apiSettings.ADMIN_USERNAME;
        const userPassword = apiSettings.ADMIN_PASSWORD;
        ...
        const userInputBody = {
            login: 'login' + Math.floor(Math.random() * (100 - 1 + 1)) + 1,
            password:
                'password' + Math.floor(Math.random() * (100 - 1 + 1)) + 1,
            email: 'email' + Math.floor(Math.random() * (100 - 1 + 1)) + 1,
        };
