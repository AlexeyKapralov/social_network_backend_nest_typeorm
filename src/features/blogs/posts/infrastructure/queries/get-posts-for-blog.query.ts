...
export class GetPostsForBlogQuery
    implements IQueryHandler<GetPostsForBlogPayload, InterlayerNotice<Post[]>>
{
    constructor(
    ...
    async execute(
        ...
        // ): Promise<InterlayerNotice<GetBlogsResultType>> {
    ): Promise<InterlayerNotice<any>> {
        // const notice = new InterlayerNotice<GetBlogsResultType>();
        const notice = new InterlayerNotice<any>();
        ...
        const posts = await this.postRepo
            .createQueryBuilder('p')
            .leftJoinAndSelect('p.blog', 'blog')
            // .leftJoinAndSelect('p.like', 'likes')
            .leftJoinAndSelect(
                '(' + likes.getQuery() + ')',
                'like',
                '"like_parentId" = p.id',
...
export type GetBlogsResultType = PaginatorDto<Post>;
